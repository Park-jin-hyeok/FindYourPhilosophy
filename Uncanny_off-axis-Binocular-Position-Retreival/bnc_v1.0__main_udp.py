...
def process_camera_feed():
    ...
    threshold = 20
    circle_radius = 15  # Radius of the circle to draw 
    circle_color = (0, 255, 0)

    screen_w = 1920 # this seems to have a better result than the high pixel
    screen_h = 1080
    ...
    with mmap.mmap(-1, shm_size, tagname='Local\\memfile', access=mmap.ACCESS_WRITE) as mm:
        ...
        while True:
            ...
            cur_img2 = camera1.capture()
            cur_img1 = camera2.capture()
            ...
            print_timer.update_cur_time()
            # check time 
            frame_duration = frame_timer.track() 
            fps = 1 / frame_duration 

            elapsed_time = print_timer.cur_time - print_timer.prev_time 

            if elapsed_time >= print_interval:
                    
                print(f'Current_frames: {fps}')
                print_timer.update_prev_time()
