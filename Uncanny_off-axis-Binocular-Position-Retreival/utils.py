...
def gausian_blur(img, blur_ksize=(20,20), blur_sigma=10):
    blured = cv2.GaussianBlur(img, blur_ksize, blur_sigma)
    return blured

def find_color_in_img_mean(img, color, threshold=1, blur_ksize=(10,10), blur_sigma=3):
...
def find_color_coordinates_hsv(frame):
    ...
    if not dual_filter:
        ...
        color_mask_1 = cv2.inRange(hsv_frame, lower_color_1, upper_color_1)
        color_mask_2 = cv2.inRange(hsv_frame, lower_color_2, upper_color_2) 
